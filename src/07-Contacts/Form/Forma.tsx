...
export const Forma = () => {
  ...
  const sendHandler = async (name: string, email: string, subj: string, message: string) => {
    await axios
      .post('https://msaserj.ru:3011/sendmessage', {
        name,
        email,
        subj,
        message
  ...
  return (
    <form onSubmit={formik.handleSubmit} className={css.form}>
      ...
      <Button type={'submit'} className={css.button} disabled={!(formik.isValid && formik.dirty)}>
        {response ? <strong className={css.sended}>{response}</strong> : 'Send message'}
      </Button>
    </form>
  );
};
